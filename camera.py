...
capture_entire_frame = True  # Set to False to capture only faces
...
while True:
  ...
  if len(faces) > 0 and current_time - last_capture_time > 2:
    if capture_entire_frame:
      filename = f"captured_frame_{current_time}.jpg"
    else:
      filename = f"detected_face_{current_time}.jpg"
